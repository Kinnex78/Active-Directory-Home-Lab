...
![DHCP enabled](screenshots/28-checking-dhcp-recievement.png)
...
While forcing a restart, the vm stucked while loading. I decided to force the restart via "ctrl + R". The process of joining the domain didnt vanished, so logging in with the admin credentials was possible by now.
